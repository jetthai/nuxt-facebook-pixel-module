...
export default (ctx, inject) => {
  ...
  const cachePixel = JSON.parse(window.localStorage.getItem('pix')).value
  ...
  // console.log(`pixelOptions: ${JSON.stringify(pixelOptions)}`)



  // console.log(`urlParams: ${urlParams} cachePixel: ${cachePixel}`)

  // console.log(`stalePixelId: ${stalePixelId}`)
  ...
  if (ctx.app && ctx.app.router) {
    ...
    router.afterEach(({ path }) => {
      ...
      const pixelOptions = Object.assign({}, matchingPixel || parsedOptions)
      // console.log(`afterEach: ${JSON.stringify(pixelOptions)}`)
      if (pixelOptions.pixelId !== instance.options.pixelId) {
        instance.setPixelId(pixelOptions.pixelId)
        // console.log(`instance set to: ${pixelOptions.pixelId}`)
      }
